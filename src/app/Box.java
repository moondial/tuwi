...
public class Box {
	...
	public Box richstr(String[] text_chunk, int[] format, int focus_at) {
		...
		for (int j = 0; j < text_chunk.length; ++j) {
			int type = format[j] >> 28;
			//Tuwi.log("t"+Integer.toHexString(format[j]));
			if(type == 0) {
				str(0, text_chunk[j]);
				continue;
			} else if(focus_at == j) {
				str(RichText.color_h[type], text_chunk[j], RichText.bgcolor[type], 1);
			} else {
				str(RichText.color[type], text_chunk[j]);
			}
			if(_y > 0)
			// y位置を記録
			format[j] = (format[j] & 0xf0000fff) | (_y & 0xffff << 12);
		}
		return this;
	}
