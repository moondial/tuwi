 ...
 *
 *  int startWholeExpr = r.getParenStart(0); // startWholeExpr will be i 1
 *  int endWholeExpr = r.getParenEnd(0);     // endWholeExpr will be i 6
 *  int lenWholeExpr = r.getParenLength(0);  // lenWholeExpr will be 5
 *
 *  int startInside = r.getParenStart(1);    // startInside will be i 1
 *  int endInside = r.getParenEnd(1);        // endInside will be i 5
 ...
 *      If it ever is fixed, URLUtils #137 in RETest.txt should be updated.</li>
 * </ul>
 *
 * </font>
 *
...
public class RE //implements Serializable
{
    ...
    /*transient*/ /*CharacterIterator*/String search;           // The string being matched against
     ...
     * @see recompile
     */
    public RE(String pattern) throws IllegalArgumentException/*RESyntaxException*/
    {
     ...
     * @see recompile
     */
    public RE(String pattern, int matchFlags) throws IllegalArgumentException/*RESyntaxException*/
    {
...
*/
    ...
    /**
     * Returns the start i of a given paren level.
     *
     * @param which Nesting level of subexpression
     * @return String i
     */
    ...
    /**
     * Returns the end i of a given paren level.
     *
     * @param which Nesting level of subexpression
     * @return String i
     */
     ...
     * @return Final input array i if match succeeded.  -1 if not.
     */
        ...
        for (int node = firstNode; node < lastNode; )
        {
            ...
            switch (opcode)
            {
                ...
                case '*':
                    {
                        ...
                        if ((idxNew = matchNodes(node + 3, 65536, idx)) != -1)
                        {
                            return idxNew;
                        }
                ...
                case '+':
                    {
                        ...
                        if ((idxNew = matchNodes(next, 65536, idx)) != -1)
                        {
                            return idxNew;
                        }
                        ...
                        node = next + (short) instruction[next + 2];
                        continue;
                    }
                ...
                case '8':
                    {
                        ...
                        return matchNodes(node + 3, next, idx);
                    }

                case '=':
                    {
                        // Continue matching the rest without using the reluctant subexpr
                        if ((idxNew = matchNodes(next + (short) instruction[next + 2], 65536, idx)) != -1)
                        {
                            return idxNew;
                        }
                ...
                case '$':
                    ...
                    if (!(0 >= search.length()) && !(idx >= search.length()))
                    {
                ...
                case '\\':
                    ...
                    switch (opdata)
                    {
                        ...
                        case 'b':
                            {
                                char cLast = ((idx == 0) ? '\n' : search.charAt(idx - 1));
                                char cNext = (((idx >= search.length())) ? '\n' : search.charAt(idx));
                                if ((isLetterOrDigit(cLast) == isLetterOrDigit(cNext)) == (opdata == 'b'))
                                {
                                    return -1;
                                }
                            }
                        ...
                        case 'S':
                            ...
                            switch (opdata)
                            {
                                case 'w':
                                case 'W':
                                    if (!((isLetterOrDigit(c) || c == '_') == (opdata == 'w')))
                                    {
                                        return -1;
                                    }
                                    break;
                ...
                case '.':
                    ...
                    else
                    {
                        ...
                        if ((idx >= search.length()) || isNewline(idx))
                        {
                            return -1;
                        }
                    }
...
*/
                ...
                case '|':
                {
                    ...
                    if (instruction[next /* + offsetOpcode */] != '|')
                    {
     ...
     * input string, starting at i i of the input string.  This method
     * is only meant for internal use.
     *
     * @param i The input string i to start matching at
...
*/
     ...
     * @return True if string matched
     */
    public boolean match(/*CharacterIterator*/String search, int i)
    {
        ...
        if ((program.flags & 2) == 2)
        {
            // Non multi-line matching with BOL: Must match at '0' i
        ...
        else
        {
            ...
            for ( ; !(i + prefix.length - 1 >= search.length()); i++)
            {
                int j = i;
                int k = 0;
    ...
    public String[] split(String s)
    {
        ...
        {//System.out.println("RE: "+getParen(0)+" "+getParen(1)+" "+getParen(2));
            ...
            // New!
            //RE date = new RE("((\\d\\d)/(\\d\\d)/(\\d\\d))");
            //String[] sp = date.split("test2 10/06/20test1");
            // sp == ["test2 ", "10/06/20", "10", "06", "20", "test1"]
            for(int i=1, l=getParenCount(); i<l; ++i)
            	v.addElement(getParen(i));
     ...
     *
     * @param before String to substitute within
     * @param after String to substitute for all matches of this regular expression.
    ...
    {
        return replace(before, after, true, true);
    }
     ...
     *
     * @param before String to substitute within
     * @param after String to substitute for matches of this regular expression
     ...
     * unchanged).
     */
    public String replace(String before, String after, boolean all, boolean bkrfs)
    {
        // String to return
        ...
        // Try a match at each position
        while (pos < len && match(before, pos))
        {
            // Append string before match
            ret.append(before.substring(pos, getParenStart(0)));
            ...
            if (bkrfs)
            {
                ...
                int lLastPosition = -2;
                int lLength = after.length();

                while ((lCurrentPosition = after.indexOf("$", lCurrentPosition)) >= 0)
                {
                    if ((lCurrentPosition == 0 || after.charAt(lCurrentPosition - 1) != '\\')
                        && lCurrentPosition + 1 < lLength)
                    {
                        char c = after.charAt(lCurrentPosition + 1);
                        if (c >= '0' && c <= '9')
                        {
                            // Append everything between the last and the current $ sign
                            ret.append(after.substring(lLastPosition + 2, lCurrentPosition));
                ...
                ret.append(after.substring(lLastPosition + 2, lLength));
            }
            else
            {
        ...
        if (pos < len)
        {
            ret.append(before.substring(pos));
        }
    ...
    // [a-zA-Z0-9_]と多バイト文字 正確には間違っているがiAppli用なので実用上問題なし。
